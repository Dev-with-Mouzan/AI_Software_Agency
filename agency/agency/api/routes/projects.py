...
async def create_project(
    payload: ProjectCreate, session: DbSession, user: CurrentUser
) -> ProjectOut:
...
async def list_projects(session: DbSession, user: CurrentUser) -> list[ProjectOut]:
    projects = await project_service.list(session, owner_id=user.id)
...
async def update_project(
    project_id: UUID, payload: ProjectUpdate, session: DbSession, user: CurrentUser
) -> ProjectOut:
    project = await require_owned_project(session, project_id, user)
...
async def add_milestone(
    project_id: UUID, payload: MilestoneIn, session: DbSession, user: CurrentUser
) -> MilestoneOut:
    await require_owned_project(session, project_id, user)
...
async def upload_plan(
    project_id: UUID, file: UploadFile, session: DbSession, user: CurrentUser
) -> PlanUploadOut:
